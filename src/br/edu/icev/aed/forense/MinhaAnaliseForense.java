...
public class MinhaAnaliseForense implements AnaliseForenseAvancada {
    ...
    public List<Alerta> priorizarAlertas(String arquivo, int n) throws IOException {
        ...
        // Criando uma PriorityQueue com um Comparator para ordenar por SEVERITY_LEVEL decrescente
        // O Comparator usa b.getSeverityLevel() - a.getSeverityLevel() para priorizar severidade maior
        Comparator<Alerta> comparatorSeveridadeDesc = (a, b) -> Integer.compare(b.getSeverityLevel(), a.getSeverityLevel());
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(arquivo))) {
            ...
            while ((linha = br.readLine()) != null) {
                ...
                try {
                    ...
                    // Trata o campo BYTES_TRANSFERRED (índice 6), que pode estar vazio em algumas linhas
                    long bytesTransferred = Long.parseLong(valores[6].isEmpty() ? "0" : valores[6]);

                    Alerta novoAlerta = new Alerta(timestamp, userId, sessionId, actionType, targetResource, severityLevel, bytesTransferred);
        ...
        // Extrai até 'n' alertas, ou até que a fila esteja vazia (tratando o Caso Específico de menos de 'n' eventos).
        for (int i = 0; i < n && !priorityQueue.isEmpty(); i++) {
            alertasPriorizados.add(priorityQueue.poll());
        }
        // 5. NUNCA retornar null. (Adicionei a palavra-chave return em outras partes para cumprir com o requisito de NUNCA retornar null)
